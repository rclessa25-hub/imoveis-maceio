...
window.validateGalleryModule = function() {
    ...
    if (!basicChecks.showPdfModal) {
        console.warn('⚠️ showPdfModal não disponível - PDFs podem não funcionar');
        
        // Criar fallback imediato se não existir
        if (!window.showPdfModal) {
            window.showPdfModal = function(propertyId) {
                console.warn('📄 Usando fallback de PDF na galeria');
                const property = window.properties?.find(p => p.id == propertyId);
            ...
            console.log('✅ Fallback showPdfModal criado');
        }
    }
...
window.initializeGalleryModule = function() {
    ...
    if (!window.showPdfModal) {
        console.warn('⚠️ showPdfModal não encontrado na inicialização da galeria');
        ...
        setTimeout(() => {
            if (!window.showPdfModal && window.PdfSystem && typeof window.PdfSystem.showModal === 'function') {
                ...
                window.showPdfModal = function(propertyId) {
                    console.warn('📄 PDF System não disponível - usando fallback básico');
                    alert('Sistema de documentos temporariamente indisponível.');
                };
                console.log('✅ Fallback básico criado');
            }
        }, 1000);
    } else {
        console.log('✅ showPdfModal já disponível');
    }
...
window.testGalleryPdfAccess = function(propertyId) {
    console.log('🧪 Testando acesso a PDFs da galeria...');
    ...
    if (typeof window.showPdfModal === 'function') {
        ...
        console.error('❌ showPdfModal não disponível');
        return false;
    }
};
